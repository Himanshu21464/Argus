...
int main() {
  ...
  for (std::size_t i = 0; i < params.size(); ++i) {
    ...
    // Convergence sanity: chain mustn't be sampling the prior. SIE
    // image positions over-determine the lens, so each marginal
    // collapses to a fraction of its prior box.
